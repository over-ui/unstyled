...
describe('Rendering Toggle', () => {
  ...
  describe('disabled rendering', () => {
    ...
    beforeEach(() => {
      ...
      toggle = utils.getByRole('button', { name: /Toggle/ });
      before = utils.getByRole('button', { name: /Before/ });
    });
...
describe('접근성과 키보드 이벤트에 문제가 없어야 한다.', () => {
  ...
  beforeEach(() => {
    ...
    toggle = utils.getByRole('button', { name: /toggle/i });
    before = utils.getByRole('button', { name: /before/i });
    after = utils.getByRole('button', { name: /after/i });
  });
